...
class AccountMove(models.Model):
    ...
    def _compute_payment_groups(self):
        ...
        """
        for rec in self:
            rec.payment_group_ids = rec._get_reconciled_payments().mapped(
                'payment_group_id')
    ...
    def action_account_invoice_payment_group(self):
        ...
        if self.state != 'posted' or self.invoice_payment_state != 'not_paid':
            raise ValidationError(_(
                'You can only register payment if invoice is posted \
                    and unpaid'))
        return {
    ...
    def action_view_payment_groups(self):
        if self.type in ('in_invoice', 'in_refund'):
            action = self.env.ref(
                'l10n_ve_account_payment.action_account_payments_group_payable')
        else:
            action = self.env.ref(
                'l10n_ve_account_payment.action_account_payments_group')
        ...
        if len(self.payment_group_ids) != 1:
            ...
            res = self.env.ref(
                'l10n_ve_account_payment.view_account_payment_group_form', False)
    ...
    def button_cancel(self):
        self.filtered(lambda x: x.state != 'draft' and 
            x.pay_now_journal_id).write({'pay_now_journal_id': False})
