...
class AccountPaymentGroup(models.Model):
    ...
    def action_payment_sent(self):
        ...
        template = self.env.ref(
            'l10n_ve_account_payment.email_template_edi_payment_group',
            False)
    ...
    def payment_print(self):
        ...
        return self.env.ref(
            'l10n_ve_account_payment.action_report_payment_group').report_action(self)
    ...
    def post(self):
        ...
        for rec in self:
            ...
            if not create_from_website and not create_from_expense:
                rec.payment_ids.sorted(
                    key=lambda l: l.signed_amount).filtered(
                        lambda x: x.state == 'draft').post()
