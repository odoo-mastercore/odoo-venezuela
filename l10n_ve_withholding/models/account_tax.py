...
class AccountTax(models.Model):
    ...
    #     if self.l10n_ve_tax_type == 'partner_tax' and payment.l10n_ve_iva == True:
    ...
    #                         }))
    #                 if distribution:
    #                     vals['l10n_ve_withholding_distribution_ids'] = distribution
    ...
    #         vals['l10n_ve_comment_withholding'] = "%s x %s" % (base_amount, alicuota)
    ...
    #     elif self.l10n_ve_tax_type == 'tabla_islr':
    ...
    #         else:
    #             regimen = payment_group.l10n_ve_regimen_islr_id
    #         vals = super(AccountTax, self).get_withholding_vals(
    #             payment_group)
    #         default_regimen_islr_id = ctx.get('default_regimen_islr_id', None)
    #         to_pay = payment_group.to_pay_move_line_ids[0]
    ...
    #         if default_regimen_islr_id:
    #             lines_base = 0
    #             for line in payment_group.l10n_ve_withholding_distributin_islr_ids:
    #                 if line.l10n_ve_regimen_islr_id == regimen:
    ...
    #                         if li.product_id.product_tmpl_id.l10n_ve_disable_islr:
    ...
    #         else:
    #             amount = withholding
    #         vals['l10n_ve_concept_withholding'] = str(regimen.code_seniat)+' - '+str(regimen.activity_name)
    #         vals['l10n_ve_comment_withholding'] = str(withholding_percentage*100)+"%"
    ...
    #     for tax in self.filtered(lambda x: x.l10n_ve_tax_type != 'none'):
    ...
    #         if payment_group.l10n_ve_withholding_distributin_islr_ids:
    #             if payment_withholding:
    #                 payment_withholdings = self.env[
    #                 'account.payment'].search([
    #                     ('payment_group_id', '=', payment_group.id),
    #                     ('tax_withholding_id.l10n_ve_tax_type', '=', 'tabla_islr'),
    #                     ('automatic', '=', True),])
    #                 for p in payment_withholdings:
    #                     p.unlink()
    #             for islr in payment_group.l10n_ve_withholding_distributin_islr_ids.mapped('l10n_ve_regimen_islr_id'):
    ...
    #                         '=', payment_method.id),
    #                         ('type', 'in', ['cash', 'bank']),
    #                         ('l10n_ve_apply_iva', '=', True),
    #                     ], limit=1)

    #                 if tax.l10n_ve_tax_type == 'tabla_islr':
    #                     journal = self.env['account.journal'].search([
    #                         ('company_id', '=', tax.company_id.id),
    #                         ('outbound_payment_method_line_ids.payment_method_id','=', payment_method.id),
    #                         ('type', 'in', ['cash', 'bank']),
    #                         ('l10n_ve_apply_islr', '=', True),
    #                     ], limit=1)
    #                 if not journal:
    #                     raise UserError(_(
    ...
    #                         and tax.l10n_ve_tax_type != 'tabla_islr':
