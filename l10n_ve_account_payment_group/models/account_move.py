...
class AccountMove(models.Model):
    ...
    def action_account_invoice_payment_group(self):
        self.ensure_one()
        if self.state != 'posted' or or self.payment_state not in [
            'not_paid','in_payment']::
            raise ValidationError(_(
                'You can only register payment if invoice is posted and unpaid'))
        return {
    ...
    def pay_now(self):
        ...
        for rec in self:
            ...
            if pay_journal and rec.state == 'posted' and rec.\
    ...
    def action_view_payment_groups(self):
        if self.type in ('in_invoice', 'in_refund'):
            ...
            action = self.env.ref(
                'l10n_ve_account_payment_group.action_account_payments_group')
        ...
        if len(self.payment_group_ids) != 1:
            ...
            res = self.env.ref(
                'l10n_ve_account_payment_group.view_account_payment_group_form', False)
