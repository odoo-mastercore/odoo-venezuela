...
class AccountMoveLine(models.Model):
    ...
    def _compute_payment_group_matched_amount(self):
        ...
        payment_lines = payments.mapped('move_line_ids').filtered(
            lambda x: x.account_internal_type in ['receivable', 'payable'])

        for rec in self:
            debit_move_amount = sum(payment_lines.mapped('matched_debit_ids').filtered(
                lambda x: x.debit_move_id == rec).mapped('amount'))
            credit_move_amount = sum(payment_lines.mapped('matched_credit_ids').filtered(
                lambda x: x.credit_move_id == rec).mapped('amount'))
